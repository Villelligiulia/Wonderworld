...
score = 0
"""Declare a repeat_game flag which will allow to loop over the same
 untill it is set to false"""
repeat_game = True
...
def introduction_game():
    """Display logo and print game rules"""
    print(logo)
...
introduction_game()
"""
 This loop will repeat the game untill the condition repeat_game
 will be set to False. This will happen when a valid but incorrect
 answer by the user will be given
 """
while repeat_game:
    def build_the_option(wonder):
        """
        Use the wonder data to build the game options
        and return a readable format.
        """
    ...
    def check_user_answer(answer, a_years, b_years):
        ...
        Call the validate_data function to validate that the answer
        given is valid, before checking that the answer given is correct
        Use if-else statement to compare the two options against the user
        answer:
        - if int wonder_a < int wonder_b (i.e it was built before),
         and the user choose 'a' return 'a' as True, if the user choose 'b'
         and wonder_b < a return 'b' as True else return False
        """
    ...
    assign the wonder_a to the wonder_b declared outside the first
    while loop on line 10, so that if the answer is correct, on the next
    iteration the correct answer will be now the first option against a new
    random wonder option
    """
    wonder_a = wonder_b
    wonder_b = random.choice(data)
    ...
    """Retrieve int year_built from wonders_data dictionary
    to get the year of each wonder"""
    wonder_a_years = wonder_a["year_built"]
    wonder_b_years = wonder_b["year_built"]
    """ Call check_user_answer function to establish
    if the user guess it's right or not"""
    ...
    -Track the score
    -When the game is over add the option to play again and start
     the game all over.
      -If the user wants to play again, clear the screen and start the game
    again.-if the user doesn't want to play anymore, quit the game,
    say 'bye' to the user and print all the description from wonders_data
    to show the user all the wonders from Ancient and Modern world in
     a readable format
    """
